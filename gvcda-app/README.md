...
checklist of what turns this from a demo into a real deployment: SMS provider +
DLT registration, hosting, and app store submission. Payments are already fully
real (see below) — no gateway account needed. The rest of the hardening (rate
limiting, security headers, OTP expiry/lockout) is done; SMS just runs in a safe
mock mode until you supply real credentials in `.env`.

## What's actually implemented

- **Auth**: phone + OTP, rate-limited (5 requests/15min) with OTP expiry (5 min) and a 3-attempt lockout. Dev mode with no SMS provider configured returns the OTP directly (`123456`, fixed, for a stable demo) — configure `MSG91_*` or `TWILIO_*` env vars for real random-OTP SMS delivery in production (`lib/sms.js`). Accounts can hold more than one role (e.g. Member + Retailer) — a **Role Switcher** re-issues the token with the new active role instead of a second login.
...
gvcda-app/
  backend/          Express API + SQLite database
    ...
    lib/             bankDetails.js + payments.js (UPI QR/bank transfer), paymentRequests.js (verification queue), sms.js (MSG91/Twilio), memberships.js
...
Open `http://localhost:5173`. Log in with one of the seeded demo numbers below — OTP is always `123456`.
...
- **Security**: `helmet` security headers, `express-rate-limit` (global + a stricter limiter on OTP requests), CORS locked to `CORS_ORIGIN` in production, request logging (`morgan`), centralized JSON error handler (no stack traces leak to clients), graceful shutdown on `SIGTERM`.
- **Auth**: `JWT_SECRET` is required from the environment in production (the server refuses to boot without it) — see `middleware/auth.js`.
...
- **SMS**: real MSG91/Twilio integration behind env-var flags — see `lib/sms.js`. Refuses to silently fall back to the fixed demo OTP once `NODE_ENV=production` is set without a provider configured, so you can't accidentally ship the demo mode.
